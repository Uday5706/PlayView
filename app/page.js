  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      switch (e.key.toLowerCase()) {
        ...
        case 'arrowup': 
          e.preventDefault();
          const nextSpeedIdx = Math.min(PLAYBACK_SPEEDS.length - 1, PLAYBACK_SPEEDS.indexOf(rateRef.current) + 1);
          updatePlaybackSpeed(PLAYBACK_SPEEDS[nextSpeedIdx]);
          break;
        case 'arrowdown': 
          e.preventDefault();
          const prevSpeedIdx = Math.max(0, PLAYBACK_SPEEDS.indexOf(rateRef.current) - 1);
          updatePlaybackSpeed(PLAYBACK_SPEEDS[prevSpeedIdx]);
          break;
        ...
        playerRef.current = new window.YT.Player("youtube-player", {
          ...
          events: {
            'onReady': (event) => {
              ...
              if (autoplayRef.current) event.target.playVideo(); // Use Ref here too
            },
            'onStateChange': (event) => {
              ...
              // STALE CLOSURE FIX: Use the Refs to calculate the next step
              if (event.data === window.YT.PlayerState.ENDED && autoplayRef.current) {
                const latestIndex = currentVideoIndexRef.current;
                const latestVideos = activeVideosRef.current;
                const nextIdx = latestIndex + 1;

                if (nextIdx < latestVideos.length) {
                  setCurrentVideo(latestVideos[nextIdx].id);
                  playerRef.current.loadVideoById(latestVideos[nextIdx].id);
                } else if (isOneShotMode) {
                  // Optional: If it's the very last video in One-Shot, ensure progress matches the end
                  setGlobalProgress(totalGlobalDuration);
                }
              }
            },
          },
          ...
          <div className="w-full bg-gray-950 p-4 lg:p-6">
            {isOneShotMode && activeVideos.length > 0 && (
              <div className="w-full">
                ...
                <div className="flex no-scrollbar overflow-x-auto gap-4 pb-4 snap-x w-full">
                  {oneShotChapters.map((chap, idx) => {
                    ...
                    return (
                      <button
                        ...
                        /* min-w-[280px] or min-w-[320px]: Forces the button to be a specific width so it doesn't crush.
                          shrink-0: Prevents Flexbox from shrinking the items to fit the screen.
                          snap-start: Aligns the item when scrolling stops.
                        */
                        className={`shrink-0 min-w-[280px] max-w-[320px] p-4 rounded-xl text-left transition-all border group snap-start flex flex-col justify-between min-h-[100px] ${
                          isActive 
                            ? 'bg-indigo-600/20 border-indigo-500/50 ring-1 ring-indigo-500 shadow-lg shadow-indigo-500/10' 
                            : 'bg-gray-800 border-gray-700 hover:border-gray-500 hover:bg-gray-700'
                        }`}
                      >
                        <div className="flex justify-between items-start mb-3">
                          <span className={`text-xs font-bold px-2.5 py-1 rounded-md transition-colors ${isActive ? 'bg-indigo-500 text-white' : 'bg-gray-900 text-gray-300 group-hover:bg-gray-800'}`}>
                            {formatSeconds(chap.globalStartTime)} - {formatSeconds(chap.globalEndTime)}
                          </span>
                        </div>
                        <p className={`text-sm font-medium line-clamp-2 transition-colors ${isActive ? 'text-indigo-200' : 'text-gray-300 group-hover:text-white'}`}>
                          {idx + 1}. {chap.title}
                        </p>
                      </button>
                    )
